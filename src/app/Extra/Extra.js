...
async function crearPolicivo(Estado_Union, Union_PDF, PDF_Union,Fecha_UnionPDF , id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear8';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_Union: Estado_Union,
                    Union_PDF: Union_PDF,
                    PDF_Union: PDF_Union,
                    Fecha_UnionPDF: Fecha_UnionPDF,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    crearPolicivo(informacion.Estado_Union, informacion.Union_PDF, informacion.PDF_Union, informacion.Fecha_UnionPDF, cedula, tipodedocumento);
