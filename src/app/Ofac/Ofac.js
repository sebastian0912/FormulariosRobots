...
async function crearOfac(Estado_OFAC, Anotacion_OFAC, PDF_OFAC, id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear3';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_OFAC: Estado_OFAC,
                    Anotacion_OFAC: Anotacion_OFAC,
                    PDF_OFAC: PDF_OFAC,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    // Crear el usuario
    crearOfac(informacion.Estado_OFAC, informacion.Anotacion_OFAC, informacion.PDF_OFAC, cedula, tipodedocumento);
