...
async function crearAdres(Estado_Procuraduria, Anotacion_Procuraduria, PDF_Procuraduria, id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear6';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_Procuraduria: Estado_Procuraduria,
                    Anotacion_Procuraduria: Anotacion_Procuraduria,
                    PDF_Procuraduria: PDF_Procuraduria,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    // Crear el usuario
    crearAdres(informacion.Estado_Procuraduria, informacion.Anotacion_Procuraduria, informacion.PDF_Procuraduria, cedula, tipodedocumento);
