...
async function crearAdres(Estado_FondoPension, Entidad_FondoPension, PDF_FondoPension, Fecha_FondoPension, id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear7';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_FondoPension: Estado_FondoPension,
                    Entidad_FondoPension: Entidad_FondoPension,
                    PDF_FondoPension: PDF_FondoPension,
                    Fecha_FondoPension: Fecha_FondoPension,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    crearAdres(informacion.Estado_FondoPension, informacion.Entidad_FondoPension, informacion.PDF_FondoPension, informacion.Fecha_FondoPension, cedula, tipodedocumento);
