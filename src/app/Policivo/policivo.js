...
async function crearPolicivo(Estado_Policivo, Anotacion_Policivo, PDF_Policivo, id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear2';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_Policivo: Estado_Policivo,
                    Anotacion_Policivo: Anotacion_Policivo,
                    PDF_Policivo: PDF_Policivo,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    // Crear el usuario
    crearPolicivo(informacion.Estado_Policivo, informacion.Anotacion_Policivo, informacion.PDF_Policivo, cedula , tipodedocumento);
