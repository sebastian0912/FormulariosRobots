...
async function crearAdres(Estado_Sisben, Tipo_Sisben, PDF_Sisben, Fecha_SisbenFecha_Sisben, id, tipodedocumento) {
    var body = localStorage.getItem('key');
    const obj = JSON.parse(body);
    const jwtToken = obj.jwt;
    console.log(jwtToken);

    const urlcompleta = urlBack.url + '/Robots/crear5';
    try {
        fetch(urlcompleta, {
            method: 'POST',
            body:
                JSON.stringify({
                    ...
                    Estado_Sisben: Estado_Sisben,
                    Tipo_Sisben: Tipo_Sisben,
                    PDF_Sisben: PDF_Sisben,
                    Fecha_Sisben: Fecha_SisbenFecha_Sisben,
...
document.getElementById('botonC').addEventListener('click', async () => {
    ...
    // Crear el usuario
    crearAdres(informacion.Estado_Sisben, informacion.Tipo_Sisben, informacion.PDF_Sisben, informacion.Fecha_Sisben, cedula, tipodedocumento);
